...
% Notat (2024-05-09): Lagt til illustrasjon av datapipeline og integrasjonslag for \aa synliggj\o re samspillet mellom arkitekturelementene.
% Notat (2024-05-11): Beskrevet datastyring og kvalitetsrutiner knyttet til fagfellekommentar DI-03.
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[node distance=2.3cm, >=LaTeX]
        \tikzset{stage/.style={draw, rounded corners=4pt, minimum height=1.9cm, text width=3.2cm, align=center, font=\small, fill=#1!18}, stage/.default=blue}
        \node (field) [stage=blue] {Feltniv\aa \\ \footnotesize Sensorer, SCADA, operat\o rinput};
        \node (edge) [stage=teal, right=2.3cm of field] {Edge- og gatewaylag \\ \footnotesize Filtrering, buffering, protokollkonvertering};
        \node (integration) [stage=green, right=2.3cm of edge] {Integrasjonslag \\ \footnotesize Meldingsk\o er, API-orkestrering, semantisk modellering};
        \node (platform) [stage=orange, right=2.3cm of integration] {Data- og analyseplattformer \\ \footnotesize Datasj\o, tidsserielager, modelltrening};
        \node (apps) [stage=purple, right=2.3cm of platform] {Forbrukere \\ \footnotesize Digital tvilling, dashboards, styringssystemer};
        \draw[->, thick] (field) -- (edge);
        \draw[->, thick] (edge) -- node[above, font=\scriptsize]{Normalisering, kvalitetssikring} (integration);
        \draw[->, thick] (integration) -- node[above, font=\scriptsize]{Distribusjon, standarder} (platform);
        \draw[->, thick] (platform) -- node[above, font=\scriptsize]{Innsikt, automatisering} (apps);
        \node (governance) [draw, dashed, rounded corners=4pt, text width=8cm, align=center, font=\scriptsize, fill=gray!12, below=1.4cm of integration] {Tverrg\aa ende styring: tilgangskontroll, hendelseslogging, datakatalog og livssyklus for modeller};
        \draw[->, dashed] (governance.north west) -- (edge.south);
        \draw[->, dashed] (governance.north) -- (integration.south);
        \draw[->, dashed] (governance.north east) -- (platform.south);
    \end{tikzpicture}%
    }
